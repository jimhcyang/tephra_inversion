...
/************************************************************
   ...
   * A large value of beta (1) places most of the particles
...
/************************End of normalization steps  ******************************************/
 ...
 /*fprintf(log_file, "[%d][%d] %.2f %.2f %g %g %g\n", i,j, x, x_norm, col_prob, part_prob, total_P); */
  ...
  //printf("in tephra calc : %lf \n", erupt->total_ash_mass);            
...
double calculate_rmse(POINT *pt, int size) {
    double sum = 0.0;
    for (int i = 0; i < size; i++) {
        double diff = pt[i].observed_mass - pt[i].calculated_mass;
        sum += diff * diff;
    }
    return sqrt(sum / size);
}

//function to estimate best-fit parameters for: total erution mass, median particle size, maximum column height, and alpha'
//using simulated annealing
RESULT simulated_annealing(ERUPTION *erupt, POINT *pt, WIND *W, STATS *stats, int size,
                           PARAM_RANGES ranges, double T_initial, double T_final, double cooling_rate, int max_iter) {
    RESULT bf;
    srand(time(NULL));
    
    /* safely initialize values to make sure best values
      are reported each time this function executes */
    double current_rmse = INFINITY; // initialize rmse's to inf
    double best_rmse = INFINITY;
    double delta_rmse = INFINITY;
    bf.total_mass = 0.0; //set most parameters to zero in bf
    bf.alpha = 0.0;
    bf.max_plume_elevation = 0.0;
    bf.median_phi = -5.0;  //grainsize never this coarse
    bf.rmse = INFINITY;  //set bf rmse to infinity

    // Initial guesses
    double current_mass = ranges.mass_min;
    double best_mass = current_mass;
    double current_alpha = ranges.alpha_min;
    double best_alpha = current_alpha;
    double current_elevation = ranges.elevation_min;
    double best_elevation = current_elevation;
    double current_phi = ranges.phi_min;
    double best_phi = current_phi;

    // Evaluate RMSE at the initial values
    erupt->total_ash_mass = current_mass;
    erupt->alpha = current_alpha;
    erupt->max_plume_elevation = current_elevation;
    erupt->mean_phi = current_phi;
    for (int i = 0; i < size; i++) {
        set_eruption_values(erupt, &W[0]);
        tephra_calc(erupt, pt + i, &W[0], stats);
    }
    current_rmse = calculate_rmse(pt, size);
    ...
    while (T > T_final && iter < max_iter) {
        // Generate neighboring solutions for parameters
        double neighbor_mass = current_mass + ((double)rand() / RAND_MAX - 0.5) * (ranges.mass_max - ranges.mass_min) * 0.1;
        double neighbor_alpha = current_alpha + ((double)rand() / RAND_MAX - 0.5) * (ranges.alpha_max - ranges.alpha_min) * 0.1;
        double neighbor_elevation = current_elevation + ((double)rand() / RAND_MAX - 0.5) * (ranges.elevation_max - ranges.elevation_min) * 0.1;
        double neighbor_phi = current_phi + ((double)rand() / RAND_MAX - 0.5) * (ranges.phi_max - ranges.phi_min) * 0.1;

        // Clamp within bounds
        if (neighbor_mass < ranges.mass_min) neighbor_mass = ranges.mass_min;
        if (neighbor_mass > ranges.mass_max) neighbor_mass = ranges.mass_max;
        if (neighbor_alpha < ranges.alpha_min) neighbor_alpha = ranges.alpha_min;
        if (neighbor_alpha > ranges.alpha_max) neighbor_alpha = ranges.alpha_max;
        if (neighbor_elevation < ranges.elevation_min) neighbor_elevation = ranges.elevation_min;
        if (neighbor_elevation > ranges.elevation_max) neighbor_elevation = ranges.elevation_max;
        if (neighbor_phi < ranges.phi_min) neighbor_phi = ranges.phi_min;
        if (neighbor_phi > ranges.phi_max) neighbor_phi = ranges.phi_max;

        // Evaluate RMSE at the new values
        erupt->total_ash_mass = neighbor_mass;
        erupt->alpha = neighbor_alpha;
        erupt->max_plume_elevation = neighbor_elevation;
        erupt->mean_phi = neighbor_phi;
        for (int i = 0; i < size; i++) {
            set_eruption_values(erupt, &W[0]);
            tephra_calc(erupt, pt + i, &W[0], stats);
        }
        double neighbor_rmse = calculate_rmse(pt, size);

        // Acceptance criteria
        delta_rmse = neighbor_rmse - current_rmse;
        if (delta_rmse < 0 || exp(-delta_rmse / T) > ((double)rand() / RAND_MAX)) {
            current_mass = neighbor_mass;
            current_alpha = neighbor_alpha;
            current_elevation = neighbor_elevation;
            current_phi = neighbor_phi;
    ...
    bf.total_mass = best_mass;
    bf.alpha = best_alpha;
    bf.max_plume_elevation = best_elevation;
    bf.median_phi = best_phi;
    bf.rmse = best_rmse;
    return bf;
}
