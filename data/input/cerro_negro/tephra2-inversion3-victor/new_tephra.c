...
int main(int argc, char *argv[]) { /* MAIN CODE STARTS HERE */
  ...
  */
  ...
  double T_initial = 100.0;      // Starting temperature
  double T_final = 1e-4;         // Final temperature
  double alpha = 0.9;            // Cooling factor
  int max_iter = 1000;           // Maximum iterations

/* specify parameter search ranges:
       alpha: 2-5
       mass: 1e10, 1e11
       max plume height: 5, 10
       median grainsize: 1, -1 (-1 phi is larger than 1 phi, but order in phi)
*/
PARAM_RANGES params = {2,5,1e10,1e11,5000,10000,-1,1};

/* do 100 inversions to try to find the uncertainty in
   the best parameter ranges
*/
printf("# eruption_mass, alpha, median phi, max plume height, rmse\n"); /* print header */
for (i=0; i<100; i++){
  //  set_eruption_values(&e, W[0]);
    RESULT optimized = simulated_annealing(&e, pt,*W, &stats, num_pts, params, T_initial, T_final, alpha, max_iter);

    printf("%lf ", optimized.total_mass); /*print the best-fit total mass*/
    printf("%lf ", optimized.alpha); /*print the best-fit alpha*/
    printf("%lf ", optimized.median_phi); /*print the best-fit  median phi */
    printf("%lf ", optimized.max_plume_elevation); /*print the best-fit  max plume height */
    printf("%lf\n", optimized.rmse); /*print the best-fit rmse */
}

/*
e.total_ash_mass = optimized.total_mass;
e.alpha = optimized.alpha;
e.max_plume_elevation = optimized.max_plume_elevation;
e.mean_phi = optimized.median_phi;
...
*/     
  ...
  for (i = 0;  i < num_pts;  i++) {
    fprintf(log_file, "[%.0f][%.0f] [%.2f]  %g ", (pt+i)->easting, (pt+i)->northing, (pt+i)->observed_mass,(pt+i)->calculated_mass) ;
    fprintf(log_file, "\n"); 
}
